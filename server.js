...
if (!idem.claimed) {
    if (idem.response?.pending) {
        return res.status(409).json({
            success: false,
            message: 'Этот заказ уже оформляется. Подождите немного и повторите попытку.'
        });
    }
...
app.get('/api/admin', adminAuth, (req, res) => {
  ...
  res.json({
    success: true, staff: req.staff, permissions: rolePermissions[role] || [], settings: store.settings, timezone: process.env.NOIRE_TIMEZONE || 'Asia/Yerevan',
    stats: publicStats,
