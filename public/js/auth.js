...
document.querySelector("#reset")?.addEventListener("click", async (e) => {
  e.preventDefault();
 const login = prompt(
    authT("Введите email или телефон")
);
  if (!login) return;
  try {
    const d = await api("/api/auth/request-reset", {
      method: "POST",
  ...
  alert(
    d.message +
    " " +
    authT("Код для разработки выводится в консоли сервера.")
);
