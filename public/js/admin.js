...
function renderBookings() {
  ...
  const activeStatuses = ['new', 'confirmed', 'arrived'];

  const rows = state.reservations
    .filter(r => activeStatuses.includes(String(r.status || '').toLowerCase()))
    .filter(r => `${r.number} ${r.name} ${r.phone} ${r.occasion || ''}`.toLowerCase().includes(q));

  $('#bookingsBody').innerHTML = rows.map(r => {
    ...
    return `
      <tr>
        ...
        <td>
          <select
            class="status-select"
            onchange="updateBooking(${r.id},this.value)"
          >
            ${bookingStatuses.map(s => `
              <option
                value="${s}"
                ${s === String(r.status).toLowerCase() ? 'selected' : ''}
              >
                ${statusLabel(s)}
              </option>
            `).join('')}
        ...
        <td>
          ...
          ${linked
        ? `<div class="linked-order">Заказ #${linked.number}</div>`
        : ''
      }
        </td>
      </tr>
    `;
  }).join('') || '<tr><td colspan="10">Активных бронирований нет.</td></tr>';
}
...
function photoFieldMarkup(value = '', name = 'image') {
  return `<label class="full">Фото с устройства<input name="${name}File" type="file" accept="image/*" data-photo-input><small class="admin-muted">Можно выбрать фото с телефона, планшета или компьютера. Максимум 12 МБ.</small></label><label class="full">Или URL фото<input name="${name}" value="${esc(value || '')}" placeholder="https://..."></label><div class="full photo-upload-preview" data-photo-preview hidden></div>`;
}
...
function renderSchedule() {
  ...
  const selectedEmployee = $('#scheduleEmployee')?.value || 'all';
  const selectedRole = $('#scheduleRole')?.value || 'all';
  ...
  if (employeeSelect) {
    const previousValue = employeeSelect.value || selectedEmployee;

    employeeSelect.innerHTML =
      '<option value="all">Все сотрудники</option>' +
      (people || [])
        .filter(e => {
          return selectedRole === 'all' || e.role === selectedRole;
        })
        .map(e => `
          <option value="${e.id}">
            ${esc(e.name)}
    ...
    if (
      [...employeeSelect.options]
        .some(o => o.value === String(previousValue))
    ) {
  ...
  const employeeFilter = employeeSelect?.value || 'all';
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (employeeFilter !== 'all') {
      rows = rows.filter(
        x => Number(x.employeeId) === Number(employeeFilter)
      );
    }
    ...
    cells.push(`
      <div
        class="schedule-day ${key === selected ? 'selected' : ''}"
        data-day="${key}"
      >
        <b>${d}</b>
      ...
      const canManage =
        ['owner', 'director', 'administrator', 'manager']
...
function openSchedule(id=null){
  ...
  const existingEmployee=existing
    ?people.find(
      e=>Number(e.id)===Number(existing.employeeId)
    )
    :null;
    ...
    <form id="scheduleForm" class="menu-form">
      ...
      <label>
        ...
        >
          ...
          <option value="manager" ${existingRole==='manager'?'selected':''}>
            Manager
          </option>
          <option value="waiter" ${existingRole==='waiter'?'selected':''}>
            Waiter
          </option>
          <option value="cook" ${existingRole==='cook'?'selected':''}>
            Повар
          </option>
          <option value="delivery" ${existingRole==='delivery'?'selected':''}>
            Courier
          </option>
          <option value="administrator" ${existingRole==='administrator'?'selected':''}>
            Administrator
          </option>
          <option value="director" ${existingRole==='director'?'selected':''}>
            Director
          </option>
        </select>
      </label>
      ...
      <label class="full">
        ...
        <select name="status">
          <option
            value="planned"
            ${!existing||existing.status==='planned'?'selected':''}
          >
            Запланирована
          </option>
  ...
  const roleSelect=$('#scheduleFormRole');
  const employeeSelect=$('#scheduleFormEmployee');

  function renderEmployeesByRole(){
    ...
    employeeSelect.innerHTML=
      ...
      filtered.map(e=>`
        <option
          value="${e.id}"
          ${Number(e.id)===Number(existing?.employeeId)?'selected':''}
        >
          ${esc(e.name)}
        </option>
      `).join('');
    ...
    if(!filtered.length){
      employeeSelect.innerHTML=
        '<option value="">Нет сотрудников этой роли</option>';
    }
  }

  roleSelect?.addEventListener(
    'change',
    renderEmployeesByRole
  );
  ...
  $('#scheduleForm').onsubmit=async ev=>{
    ...
    try{
      ev.target
        .querySelectorAll(
          'input[data-noire-time]'
        )
        .forEach(x=>{
          if(window.noireNormalizeTime){
            x.value=
              noireNormalizeTime(x.value)||
              x.value;
          }
        });
