...
function fillLanguageSelect(){
  ...
  if(!el)return;
  el.innerHTML=SITE_LANGUAGES.map(([code,name])=>`<option value="${code}">${name}</option>`).join('');
...
function renderAll(){
  ...
  if(state.settings){
    ...
    if($('#siteSubtitle'))$('#siteSubtitle').value=state.settings.siteSubtitle||'';
    fillLanguageSelect();
  }
  ...
  const mine=state.staff?.id&&state.staff.id!=='owner'
    ?state.employees.find(e=>Number(e.id)===Number(state.staff.id))
    :null;

  if(mine){
    const st=mine.stats||{};
    [['day','myDayRevenue','myDayOrders'],['week','myWeekRevenue','myWeekOrders'],['month','myMonthRevenue','myMonthOrders'],['year','myYearRevenue','myYearOrders']]
      .forEach(([k,rev,ord])=>{
        if($('#'+rev))$('#'+rev).textContent=money(st[k]?.revenue);
        if($('#'+ord))$('#'+ord).textContent=`${st[k]?.orders||0} выполненных заказов`
      });

    if($('#myShift'))$('#myShift').textContent=mine.shiftActive?'Смена открыта':'Смена закрыта';
  }

  $('#dashboardOrders').innerHTML=state.orders.slice(0,5).map(o=>
  ...
  $('#topItems').innerHTML=state.popularItems.map((x,i)=>
...
function renderOrders(){
  ...
  const assignedIdFor=(o,r)=>
    r==='waiter'
      ...
      :r==='delivery'
        ...
        :r==='cook'
          ?o.kitchenId
          :(o.waiterId||o.deliveryId||o.kitchenId||o.employeeId||o.staffId);
  ...
  const card=o=>{
    ...
    const staff=state.employees.find(e=>Number(e.id)===Number(assignedId));
    ...
    const staffOptions=state.employees.filter(e=>{
      ...
      return o.orderType==='waiter'
        ...
        :o.orderType==='delivery'
          ?er==='delivery'
          :['cook','manager','administrator','director','owner'].includes(er);
    });

    return `<article class="order-admin-card" onclick="openOrder(${o.id})">
      <div class="order-card-top">
        <div>
          <span class="admin-eyebrow">${esc(type(o).toUpperCase())}</span>
          <h3>#${o.number}</h3>
          <small>${(window.noireFormatDateTime?noireFormatDateTime(o.createdAt):new Date(o.createdAt).toLocaleString())}</small>
        </div>
        <span class="order-status-badge">${esc(statusLabel(o.status))}</span>
      </div>

      <div class="order-card-client">
        <b>${esc(o.customer?.name||'Гость')}</b>
        <span>${esc(o.customer?.phone||'')}</span>
      </div>

      <div class="order-card-items">${items||'Нет позиций'}</div>

      <div class="order-card-bottom">
        <strong>${money(o.total)}</strong>
        <span>${staff?`Ответственный: ${esc(staff.name)}`:'Не принят'}</span>
      </div>

      <div class="order-card-comment">
        <small>Комментарий</small>
        <p>${esc(comment)}</p>
      </div>

      <div class="order-card-actions" onclick="event.stopPropagation()">
        ${canClaim?`<button class="small-btn" onclick="claimOrder(${o.id})">Принять заказ</button>`:''}
        ${worker&&isMine&&active(o)?`
          <button class="small-btn" onclick="completeMyOrder(${o.id})">Заказ завершён</button>
          <button class="small-btn danger" onclick="cancelMyOrder(${o.id})">Отменить</button>
        `:''}
        ${!worker?`
          <select class="status-select" onchange="updateOrder(${o.id},this.value)">
            ${orderStatuses.map(st=>`
              <option value="${st}" ${st===String(o.status).toLowerCase()?'selected':''}>
                ${statusLabel(st)}
              </option>
            `).join('')}
          </select>

          <select class="status-select staff-select" onchange="assignOrder(${o.id},this.value)">
            <option value="">Сотрудник…</option>
            ${staffOptions.map(e=>`
              <option value="${e.id}" ${Number(managementAssignee(o))===Number(e.id)?'selected':''}>
                ${esc(e.name)} · ${esc((window.noireRoleLabel?noireRoleLabel(e.role):e.role))}
              </option>
            `).join('')}
          </select>
        `:''}
  ...
  if(worker){
    ...
    all=role==='cook'?rows.filter(active):rows.filter(o=>active(o)&&!assigned(o));
    mine=rows.filter(o=>Number(assigned(o))===Number(state.staff?.id)&&active(o));
    completed=rows.filter(o=>Number(assigned(o))===Number(state.staff?.id)&&!active(o));
  }else{
  ...
  const set=(id,items,empty)=>{
    const b=$('#'+id);
    if(b)b.innerHTML=items.map(card).join('')||`<div class="empty-admin-state"><h3>${empty}</h3></div>`;
  };

  set('ordersAllCards',all,'Доступных заказов нет');
  set('ordersMineCards',mine,worker?'Вы пока не приняли заказов':'');
  set('ordersCompletedCards',completed,worker?'У вас нет завершённых заказов':'Выполненных заказов нет');

  const mineWrap=$('#ordersMineWrap');
  const allWrap=$('#ordersAllWrap');
  const completedWrap=$('#ordersCompletedWrap');
...
function openOrder(id){
  ...
  const items=(o.items||[]).map(i=>
    `<div class="mini-row">
      <span><b>${esc(i.name)}</b> ×${i.quantity}</span>
      <strong>${money(Number(i.price)*Number(i.quantity))}</strong>
    </div>`
  ).join('');

  openModal(`
    <span class="admin-eyebrow">ORDER #${o.number}</span>
    <h2>${esc(o.customer?.name||'Гость')}</h2>

    <div class="detail-grid">
      <div><small>Телефон</small><b>${esc(o.customer?.phone||'—')}</b></div>
      <div><small>Тип</small><b>${esc(String(o.orderType||'delivery').toUpperCase())}</b></div>
      <div><small>Статус</small><b>${esc(statusLabel(o.status))}</b></div>
      <div><small>Дата и время</small><b>${(window.noireFormatDateTime?noireFormatDateTime(o.createdAt):new Date(o.createdAt).toLocaleString())}</b></div>
      <div><small>Ответственный</small><b>${esc(staff?.name||'Не назначен')}</b></div>
      <div><small>Адрес</small><b>${esc(o.customer?.address||'—')}</b></div>
      <div><small>Время доставки</small><b>${esc(o.customer?.deliveryTime||o.deliveryTime||'—')}</b></div>
      <div><small>Оплата</small><b>${esc(o.customer?.payment||o.payment||'—')}</b></div>
      <div><small>Стол</small><b>${o.tableId?`T${String(o.tableId).padStart(2,'0')}`:'—'}</b></div>
      <div class="full"><small>Комментарий клиента</small><b>${esc(o.customer?.comment||o.comment||'—')}</b></div>
    </div>

    <h3>Состав заказа</h3>
    <div class="history-detail-list">${items||'<p>Нет позиций.</p>'}</div>
...
function renderBookings(){
  ...
  body.innerHTML=rows.map(r=>{
    ...
    return `<tr>
      <td>
        <b>#${r.number}</b><br>
        <small>${(window.noireFormatDateTime?noireFormatDateTime(r.createdAt):new Date(r.createdAt).toLocaleString())}</small>
      </td>
      ...
      <td>${esc(window.noireFormatDate?noireFormatDate(r.date):r.date)}</td>
      <td>${esc(r.time)}</td>
      <td>${r.guests}</td>
      <td>${r.tableId?`T${String(r.tableId).padStart(2,'0')}`:'—'}</td>
      <td>${r.budget?money(r.budget):'—'}</td>

      <td>
        ${esc(r.occasion||'—')}<br>
        <small>${esc(r.comment||'')}</small>
      </td>

      <td>
        <select class="status-select" onchange="updateBooking(${r.id},this.value)">
          ${bookingStatuses.map(s=>`
            <option value="${s}" ${s===String(r.status).toLowerCase()?'selected':''}>
              ${statusLabel(s)}
            </option>
          `).join('')}
        </select>
      ...
      <td>
        <button class="small-btn" onclick="openBooking(${r.id})">Детали</button>
        ${linked?`<div class="linked-order">Заказ #${linked.number}</div>`:''}
      </td>
    </tr>`
...
function renderMenu(){
  ...
  const box=$('#menuAdminGrid');
  if(!box)return;

  box.innerHTML=state.menu.map(i=>
    `<article class="admin-product">
      <img src="${esc(i.image||'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&w=900&q=80')}" alt="${esc(i.name)}">
      ...
      <div class="admin-product-body">
        ...
        <div class="admin-product-meta">
          ...
          <small>${cats[i.category]||i.category}${i.popular?' · ★':''}</small>
        </div>

        <div class="admin-product-actions">
          <button class="small-btn" onclick="openMenu(${i.id})">Изменить</button>
          <button class="small-btn danger" onclick="deleteMenu(${i.id})">Удалить</button>
        </div>
      </div>
    </article>`
...
function renderGallery(){
  const box=$('#galleryAdminGrid');
  if(!box)return;

  box.innerHTML=state.gallery.map(i=>
    `<article class="gallery-admin-card">
      <img src="${esc(i.image)}" alt="${esc(i.title)}">
      <div class="gallery-admin-info">
        <span class="admin-eyebrow">${esc(i.category)}</span>
        <h3>${esc(i.title)}</h3>
      </div>
      <button class="small-btn danger" onclick="deleteGallery(${i.id})">Удалить</button>
    </article>`
  ).join('')
}
...
function renderTables(){
  ...
  floor.innerHTML=state.tables.map(t=>{
    ...
    return `<button class="admin-table-seat ${status==='occupied'?'busy':status==='service'?'service':status==='reserved'?'reserved':''}" style="left:${t.x}%;top:${t.y}%" onclick="openTable(${t.id})">
      <div>
        <b>${esc(t.name)}</b>
        <small>${t.seats} места · ${tableStatusLabel(status)}</small>
      </div>
    </button>`
  }).join('')
}
...
function renderCustomers(){
  ...
  box.innerHTML=customers.map(c=>{
    ...
    return `<article class="customer-admin-card">
      ...
      <div class="customer-avatar" onclick="openCustomer(${c.id})">
      ...
      <div class="customer-main" onclick="openCustomer(${c.id})">
        ...
        <div class="customer-stats">
          ...
          <span>${bookings.length} броней</span>
          <strong>${money(revenue)}</strong>
        </div>
      </div>

      <button type="button" class="customer-delete-btn" onclick="deleteCustomer(${c.id},event)">
        Удалить
      </button>
    </article>`
...
async function deleteSelectedCustomers(){
  ...
  try{
    const results=await Promise.all(
      ids.map(id=>api(`/api/admin/customers/${encodeURIComponent(id)}`,{method:'DELETE'}))
    );
...
function openCustomer(id){
  ...
  openModal(`
    <span class="admin-eyebrow">CLIENT PROFILE</span>
    <h2>${esc(c.name||'Клиент')}</h2>

    <div class="detail-grid">
      <div><small>Телефон</small><b>${esc(c.phone||'—')}</b></div>
      <div><small>Email</small><b>${esc(c.email||'—')}</b></div>
      <div><small>Регистрация</small><b>${c.registered===false?'Гостевой контакт':'Зарегистрирован'}</b></div>
      <div><small>Заказов</small><b>${orders.length}</b></div>
      <div><small>Бронирований</small><b>${bookings.length}</b></div>
    </div>
    ...
    <div class="history-detail-list">
      ${orders.map(o=>
        `<button class="client-detail-row" onclick="openOrder(${o.id})">
          <span><b>#${o.number}</b> · ${esc(statusLabel(o.status))}</span>
          <strong>${money(o.total)}</strong>
        </button>`
      ).join('')||'<p>Заказов нет.</p>'}
    </div>
    ...
    <div class="history-detail-list">
      ${bookings.map(r=>
        `<div class="mini-row">
          <span><b>#${r.number}</b> · ${esc(window.noireFormatDate?noireFormatDate(r.date):r.date)} ${esc(r.time)}</span>
          <span>${esc(statusLabel(r.status))}</span>
        </div>
        <div class="client-comment">${esc(r.comment||'Без комментария')}</div>`
...
function renderAnalytics(){
  ...
  const salesBars=$('#salesBars');
  if(salesBars){
    salesBars.innerHTML=days.map(x=>
      `<div class="bar" style="height:${Math.max(8,x.revenue/max*210)}px">
        <b>${x.revenue?money(x.revenue).replace(' ֏',''):0}</b>
        <span>${x.label}</span>
      </div>`
  ...
  if(customersList){
    customersList.innerHTML=state.customers.slice(0,8).map(c=>
      `<div class="mini-row">
        <span>${esc(c.name||'Гость')}</span>
        <small>${esc(c.phone||c.email||'')}</small>
      </div>`
    ).join('')||'<p>Нет клиентов.</p>';
  }

  const analyticsTop=$('#analyticsTop');
  if(analyticsTop){
    analyticsTop.innerHTML=(state.analytics?.topItems||[]).slice(0,8).map((x,i)=>
      `<div class="rank-row">
        <span><b>${i+1}.</b> ${esc(x.name)}</span>
        <small>${x.quantity} шт.</small>
      </div>`
    ).join('')||'<p>Нет данных.</p>';
  }
  ...
  if(categoryList){
    categoryList.innerHTML=(state.analytics?.topCategories||[]).slice(0,8).map(x=>
      `<div class="mini-row">
...
async function readImageFile(file){
  ...
  try{
    const img=await new Promise((resolve,reject)=>{
      ...
      im.onerror=()=>reject(new Error('Формат фото не поддерживается браузером'));
      im.src=raw
    });
    ...
    while(
      out.length>Math.floor(2.2*1024*1024*4/3)&&
      quality>.55
    ){
      quality-=.07;
    ...
    if(/^data:image\/(jpeg|jpg|png|webp|gif|avif);base64,/i.test(String(raw)))
      return raw;
...
function photoFieldMarkup(value='',name='image'){
  return `
    <label class="full">
      Фото с устройства
      <input name="${name}File" type="file" accept="image/*" data-photo-input>
      <small class="admin-muted">
        Можно выбрать фото с телефона, планшета или компьютера. Максимум 12 МБ.
      </small>
    </label>

    <label class="full">
      Или URL фото
      <input name="${name}" value="${esc(value||'')}" placeholder="https://...">
    </label>

    <div class="full photo-upload-preview" data-photo-preview hidden></div>
  `
}
...
function openMenu(id=null){
  ...
  openModal(`
    <span class="admin-eyebrow">NOIRÉ · MENU</span>
    <h2>${id?'Редактировать позицию':'Добавить позицию'}</h2>

    <form id="menuForm" class="menu-form">
      <label>Название
        <input name="name" value="${esc(i.name||'')}" required>
      </label>

      <label>Цена
        <input name="price" type="number" value="${i.price||''}" min="0" required>
      </label>

      <label>Категория
        <select name="category">
          ${['coffee','tea','breakfast','snacks','food','desserts','drinks','beer','sauces'].map(c=>
            `<option ${c===i.category?'selected':''} value="${c}">${c}</option>`
          ).join('')}
        </select>
      </label>

      ${photoFieldMarkup(i.image||'','image')}

      <label class="full">
        Описание
        <textarea name="description">${esc(i.description||'')}</textarea>
      </label>

      <label class="check full">
        <input type="checkbox" name="popular" ${i.popular?'checked':''}>
        Показывать как популярное
      </label>

      <div class="full">
        <button class="admin-primary">Сохранить</button>
      </div>
    </form>
  `);
  ...
  form.onsubmit=async e=>{
    ...
    try{
      ...
      await api(
        id?`/api/admin/menu/${id}`:'/api/admin/menu',
        {
          method:id?'PUT':'POST',
...
function openGallery(){
  openModal(`
    <span class="admin-eyebrow">SPACE ONLY</span>
    <h2>Добавить фото NOIRÉ</h2>

    <form id="galleryForm" class="menu-form">
      <label>Название
      ...
      ${photoFieldMarkup('','image')}

      <div class="full">
        <button class="admin-primary">Добавить</button>
      </div>
    </form>
  `);
  ...
  form.onsubmit=async e=>{
    ...
    try{
      ...
      if(!body.image)
        throw new Error('Выберите фото или укажите URL');
...
function openTable(id){
  ...
  openModal(`
    <span class="admin-eyebrow">NOIRÉ FLOOR PLAN</span>
    <h2>${esc(t.name)}</h2>

    <div class="detail-grid">
      <div><small>Статус</small><b>${tableStatusLabel(t.computedStatus||t.status)}</b></div>
      <div><small>Мест</small><b>${t.seats}</b></div>
      <div><small>Зона</small><b>${esc(t.zone||'—')}</b></div>
      <div><small>Бронь</small><b>${r?`#${r.number} · ${esc(r.name)} · ${esc(r.time)}`:'Нет'}</b></div>
      <div class="full"><small>Заказ</small><b>${o?`#${o.number} · ${money(o.total)} · ${statusLabel(o.status)}`:'Нет'}</b></div>
    </div>

    <div class="modal-actions">
      <button class="small-btn" onclick="setTableStatus(${t.id},'occupied')">Занять</button>
      <button class="small-btn" onclick="setTableStatus(${t.id},'service')">В сервис</button>
      <button class="small-btn" onclick="setTableStatus(${t.id},'available')">Освободить</button>
    </div>
  `)
}
...
function openNewBooking(){
  if(!['owner','director','administrator','manager'].includes(state.staff?.role)){
  ...
  openModal(`
    <span class="admin-eyebrow">RESERVATION · PHONE</span>
    <h2>Новое бронирование</h2>

    <form id="adminBookingForm" class="menu-form">
      <label>Имя
...
<input id="scheduleDate" name="date" type="hidden" value="${date}">
      ...
      <label>Стол
        <select name="tableId">
          ...
          ${people.map(t=>
            `<option value="${t.id}">${esc(t.name)} · ${t.seats} мест</option>`
          ).join('')}
        </select>
      </label>
      ...
      <div class="full">
        <button class="admin-primary">Создать бронь</button>
      </div>
    </form>
  `);
  ...
  $('#adminBookingForm').onsubmit=async ev=>{
    ...
    if(window.noireNormalizeTime)
      ev.target.time.value=noireNormalizeTime(ev.target.time.value)||ev.target.time.value;
...
function openBooking(id){
  ...
  openModal(`
    <span class="admin-eyebrow">RESERVATION #${r.number}</span>
    <h2>${esc(r.name)}</h2>

    <div class="detail-grid">
      <div><small>Телефон</small><b>${esc(r.phone)}</b></div>
      <div><small>Дата</small><b>${esc(window.noireFormatDate?noireFormatDate(r.date):r.date)}</b></div>
      <div><small>Время</small><b>${esc(r.time)}</b></div>
      <div><small>Гостей</small><b>${r.guests}</b></div>
      <div><small>Стол</small><b>${r.tableId?`T${String(r.tableId).padStart(2,'0')}`:'Не выбран'}</b></div>
      <div><small>Бюджет</small><b>${r.budget?money(r.budget):'—'}</b></div>
      <div class="full"><small>Повод</small><b>${esc(r.occasion||'—')}</b></div>
      <div class="full"><small>Комментарий</small><b>${esc(r.comment||'—')}</b></div>
    </div>

    <h3>Связанные заказы</h3>

    <select id="bookingOrderSelect" class="admin-search">
      <option value="">Не привязан</option>
      ${orders.map(o=>
        `<option value="${o.id}" ${Number(r.orderId)===Number(o.id)?'selected':''}>
          #${o.number} · ${money(o.total)} · ${esc(o.customer?.name||'')}
        </option>`
      ).join('')}
    </select>

    <button class="admin-primary" onclick="linkBookingOrder(${r.id},document.querySelector('#bookingOrderSelect').value)">
      Сохранить связь
    </button>
  `)
}
...
function openEmployee(id=null){
  ...
  const canManage=['owner','director','administrator'].includes(state.staff?.role);
  ...
  openModal(`
    <span class="admin-eyebrow">NOIRÉ TEAM</span>
    <h2>${id?'Редактировать сотрудника':'Новый сотрудник'}</h2>

    <form id="employeeForm" class="menu-form">
      <label>Имя
        <input name="firstName" value="${esc(e.firstName||e.name||'')}" required>
      </label>

      <label>Фамилия
        <input name="lastName" value="${esc(e.lastName||'')}" required>
      </label>

      <label>Отчество
        <input name="middleName" value="${esc(e.middleName||'')}">
      </label>

      <label>Логин
        <input name="username" value="${esc(e.username||'')}" required>
      </label>

      <label>Пароль
        <div class="password-field">
          <input name="password" type="password" minlength="8" placeholder="${id?'Оставьте пустым, если не меняете':''}" ${id?'':'required'}>
          <button type="button" class="password-toggle" aria-label="Показать пароль" aria-pressed="false">
      ...
      <label>Роль
        <select name="role">
          ${['director','administrator','manager','waiter','cook','delivery'].map(r=>
            `<option value="${r}" ${r===e.role?'selected':''}>
              ${window.noireRoleLabel?noireRoleLabel(r):r}
            </option>`
          ).join('')}
        </select>
      </label>
      ...
      <label>Телефон
        <input name="phone" value="${esc(e.phone||'')}">
      </label>

      ${photoFieldMarkup(e.photo||'','photo')}

      <label class="check full">
        <input type="checkbox" name="active" ${e.active!==false?'checked':''}>
        Аккаунт активен
      </label>

      <div class="full">
        <button class="admin-primary">${id?'Сохранить изменения':'Создать аккаунт'}</button>
      </div>
    </form>
  `);
  ...
  $('#employeeForm').onsubmit=async ev=>{
    ...
    try{
      await api(
        id?`/api/admin/employees/${id}`:'/api/admin/employees',
        {
...
function openDeliveryOrder(){
  ...
  openModal(`
    <span class="admin-eyebrow">DELIVERY · NEW ORDER</span>
    <h2>Новая доставка</h2>

    <form id="deliveryOrderForm" class="menu-form">
      <label>Имя клиента
        <input name="name" required>
      </label>
      ...
      <div class="full" id="deliveryItems"></div>

      <div class="full">
        <button type="button" class="small-btn" id="addDeliveryItem">
          + Добавить позицию
        </button>
      </div>

      <div class="full">
        <strong>Итого: <span id="deliveryTotal">0 ֏</span></strong>
      </div>

      <div class="full">
        <button class="admin-primary">Создать доставку</button>
      </div>
    </form>
  `);
  ...
  const render=()=>{
    $('#deliveryItems').innerHTML=items.map((x,i)=>
      `<div class="menu-form-row">
        <select data-di="${i}">
          ${state.menu.map(m=>
            `<option value="${m.id}" ${Number(m.id)===Number(x.id)?'selected':''}>
              ${esc(m.name)} · ${money(m.price)}
            </option>`
          ).join('')}
        </select>

        <input data-dq="${i}" type="number" min="1" max="50" value="${x.quantity}">

        <button type="button" class="small-btn danger" data-dr="${i}">×</button>
      </div>`
    ...
    $('#deliveryTotal').textContent=money(
      items.reduce(
        (a,x)=>
          a+
          Number(state.menu.find(m=>Number(m.id)===Number(x.id))?.price||0)*
          x.quantity,
        0
      )
    );
    ...
    document.querySelectorAll('[data-dq]').forEach(el=>
      el.oninput=()=>{
        items[Number(el.dataset.dq)].quantity=
          Math.max(1,Math.min(50,Number(el.value)||1));
        render()
      }
    );
...
function openWaiterOrder(){
  ...
  openModal(`
    <span class="admin-eyebrow">WAITER · NEW ORDER</span>
    <h2>Новый заказ в зале</h2>

    <form id="waiterOrderForm" class="menu-form">
      <label>Имя гостя
        <input name="name" value="Гость">
      </label>
      ...
      <div class="full" id="waiterItems"></div>

      <div class="full">
        <button type="button" class="small-btn" id="addWaiterItem">
          + Добавить позицию
        </button>
      </div>

      <div class="full">
        <strong>Итого: <span id="waiterTotal">0 ֏</span></strong>
      </div>

      <div class="full">
        <button class="admin-primary">Создать заказ</button>
      </div>
    </form>
  `);
  ...
  const renderItems=()=>{
    $('#waiterItems').innerHTML=items.map((x,i)=>
      `<div class="menu-form-row">
        <select data-item="${i}">
          ${state.menu.map(m=>
            `<option value="${m.id}" ${Number(m.id)===Number(x.id)?'selected':''}>
        ...
        <input data-qty="${i}" type="number" min="1" max="50" value="${x.quantity}">

        <button type="button" class="small-btn danger" data-remove="${i}">
          ×
        </button>
      </div>`
    ).join('')||'<p class="admin-muted">Добавьте блюда.</p>';
    ...
    document.querySelectorAll('[data-qty]').forEach(el=>
      el.oninput=()=>{
        items[Number(el.dataset.qty)].quantity=
          Math.max(1,Math.min(50,Number(el.value)||1));
        renderItems()
      }
    );
...
function renderHistory(){
  ...
  const years=[
    ...new Set(
      history
        .map(h=>String(h.month||'').slice(0,4))
        .filter(Boolean)
    )
  ...
  box.innerHTML=filtered.map(h=>{
    ...
    return `<article class="history-card">
      <div class="history-card-head">
        <div>
          <span class="admin-eyebrow">${esc(h.month||'')}</span>
          <h3>
            ${esc(
              h.month
                ...
                ).format(new Date(h.month+'-01T12:00:00'))
                :(h.label||'')
            )}
          </h3>
          <small>
            Архив создан: ${new Date(h.archivedAt).toLocaleString('ru-RU')}
          </small>
        </div>
      </div>

      <div class="history-metrics">
        <div><small>Выручка</small><b>${money(st.revenue)}</b></div>
        <div><small>Заказы</small><b>${st.orders||0}</b></div>
        <div><small>Брони</small><b>${st.reservations||0}</b></div>
        <div><small>Средний чек</small><b>${money(st.avgCheck)}</b></div>
      </div>

      <div class="history-card-actions">
        <button class="small-btn" onclick="openHistory('${esc(h.id)}')">
          Посмотреть данные
        </button>

        <button class="small-btn danger" onclick="deleteHistory('${esc(h.id)}')">
          Удалить архив
        </button>
      </div>
    </article>`
  ...
  `<div class="history-empty">
    ...
    <p>
      Когда месяц закончится, нажмите «Архивировать прошлый месяц».
      Данные сохранятся здесь и исчезнут из текущих заказов и прошедших бронирований.
    </p>
  </div>`;
}
...
function openHistory(id){
  ...
  openModal(`
    <span class="admin-eyebrow">ARCHIVE · ${esc(h.month)}</span>
    ...
    <div class="detail-grid">
      <div><small>Выручка</small><b>${money(st.revenue)}</b></div>
      <div><small>Заказы</small><b>${orders.length}</b></div>
      <div><small>Брони</small><b>${reservations.length}</b></div>
      <div><small>Клиенты в архиве</small><b>${(h.customers||[]).length}</b></div>
    </div>
    ...
    <div class="history-detail-list">
      ${orders.map(o=>
        `<div class="mini-row">
          <span><b>#${o.number}</b> · ${esc(o.customer?.name||'Гость')}</span>
          <small>
            ${money(o.total)} ·
            ${(window.noireFormatDateTime?noireFormatDateTime(o.createdAt):new Date(o.createdAt).toLocaleString())}
          </small>
        </div>`
    ...
    <div class="history-detail-list">
      ${reservations.map(r=>
        `<div class="mini-row">
          <span><b>#${r.number}</b> · ${esc(r.name||'Гость')}</span>
          <small>
            ${esc(window.noireFormatDate?noireFormatDate(r.date):r.date)}
            ${esc(r.time||'')} ·
...
async function archivePreviousMonth(){
  if(!confirm('Перенести прошлый месяц в историю? Его заказы и прошедшие бронирования исчезнут из текущей базы и будут сохранены в архиве.'))return;
  ...
  try{
    ...
    state.orders=state.orders.filter(o=>
      !d.archive.orders.some(a=>Number(a.id)===Number(o.id))
    );

    state.reservations=state.reservations.filter(r=>
      !d.archive.reservations.some(a=>Number(a.id)===Number(r.id))
    );
...
async function deleteHistory(id){
  if(!confirm('Удалить этот архив? Данные текущих заказов и бронирований не изменятся.'))return;
...
async function clearHistory(){
  if(!confirm('Удалить ВСЮ историю архивов? Это действие нельзя отменить.'))return;
...
function renderEmployees(){
  ...
  box.innerHTML=(state.employees||[]).map(e=>{
    const canShift=
      canManage||
      Number(e.id)===Number(state.staff?.id);

    return `<article class="employee-card">
      <div class="employee-avatar">${esc((e.name||'?')[0])}</div>

      <div>
        <h3>${esc(e.name)}</h3>
        <p>@${esc(e.username)}</p>
        <span>${esc(window.noireRoleLabel?noireRoleLabel(e.role):e.role)}</span>
      </div>

      <div class="employee-shift ${e.shiftActive?'on':''}">
        <i></i>
        ${e.shiftActive?'На смене':'Не работает'}
      </div>

      <div class="employee-metrics">
        <div>
          <small>День</small>
          <b>${money(e.stats?.day?.revenue)}</b>
          <span>${e.stats?.day?.orders||0} заказов</span>
        </div>

        <div>
          <small>Неделя</small>
          <b>${money(e.stats?.week?.revenue)}</b>
          <span>${e.stats?.week?.orders||0} заказов</span>
        </div>

        <div>
          <small>Месяц</small>
          <b>${money(e.stats?.month?.revenue)}</b>
          <span>${e.stats?.month?.orders||0} заказов</span>
        </div>
      </div>

      <small class="employee-contact">
        ${esc(e.email||e.phone||'Контакты не указаны')}
      </small>

      <div class="employee-actions">
        ${canShift?(
          e.shiftActive
            ?`<button class="small-btn" onclick="shift(${e.id},'stop')">Закрыть смену</button>`
            :`<button class="small-btn" onclick="shift(${e.id},'start')">Начать смену</button>`
        ):''}

        ${canManage?`
          <button class="small-btn" onclick="openEmployee(${e.id})">
            Изменить
          </button>

          <button class="small-btn danger" onclick="removeEmployee(${e.id})">
            Удалить
          </button>
        `:''}
      </div>
...
function renderSchedule(){
  ...
  const selected=$('#scheduleDate')?.value||dateNow();
  const base=new Date(selected+'T12:00:00');

  const year=base.getFullYear();
  const month=base.getMonth();

  const emp=$('#scheduleEmployee')?.value||'all';
  const role=$('#scheduleRole')?.value||'all';
  ...
  if($('#scheduleEmployee')){
    $('#scheduleEmployee').innerHTML=
      '<option value="all">Все сотрудники</option>'+
      (people||[]).map(e=>
        `<option value="${e.id}">${esc(e.name)}</option>`
      ).join('');

    $('#scheduleEmployee').value=emp;
  }
  ...
  for(let d=1;d<=daysInMonth;d++){
    ...
    let rows=(state.shifts||[]).filter(x=>x.date===key);

    if(emp!=='all')
      rows=rows.filter(
        x=>Number(x.employeeId)===Number(emp)
      );
    ...
    if(role!=='all')
      rows=rows.filter(
        x=>
          (people||[]).find(
            e=>Number(e.id)===Number(x.employeeId)
          )?.role===role
      );
    ...
    cells.push(`
      <div class="schedule-day ${key===selected?'selected':''}" data-day="${key}">
        <b>${d}</b>

        ${rows.slice(0,4).map(x=>{
          const e=(people||[]).find(
            z=>Number(z.id)===Number(x.employeeId)
          );

          return `<div class="schedule-mini">
            <span>${esc(e?.name||'Сотрудник')}</span>
            <small>${esc(x.start)}–${esc(x.end)}</small>

            ${['owner','director','administrator','manager'].includes(state.staff?.role)
              ?`<button type="button" onclick="event.stopPropagation();openSchedule(${x.id})">✎</button>`
              :''}
          </div>`
        }).join('')}

        ${rows.length>4?`<em>+${rows.length-4}</em>`:''}
      </div>
    `)
  }
  ...
  box.querySelectorAll('[data-day]').forEach(el=>
    el.onclick=()=>{
      if($('#scheduleDate'))
        $('#scheduleDate').value=el.dataset.day;
...
function openSchedule(id=null){
  ...
  openModal(`
    ...
    <h2>
      ${existing?'Редактировать':'Добавить'} плановую смену
    </h2>
  ...
  <input id="scheduleDate" name="date" type="hidden" value="${date}">
      ...
      <label>Сотрудник
        <select name="employeeId" required>
          ${people.map(e=>
            `<option value="${e.id}" ${Number(e.id)===Number(existing?.employeeId)?'selected':''}>
              ${esc(e.name)} · ${esc((window.noireRoleLabel?noireRoleLabel(e.role):e.role))}
            </option>`
          ).join('')}
        </select>
      ...
      <label>Начало
        <input name="start" data-noire-time inputmode="numeric" placeholder="10:00" value="${existing?.start||'10:00'}" required>
      </label>

      <label>Окончание
        <input name="end" data-noire-time inputmode="numeric" placeholder="18:00" value="${existing?.end||'18:00'}" required>
      </label>

      <label class="full">Статус
        <select name="status">
          <option value="planned" ${!existing||existing.status==='planned'?'selected':''}>
            Запланирована
          </option>

          <option value="cancelled" ${existing?.status==='cancelled'?'selected':''}>
            Отменена
          </option>
        </select>
      </label>
  ...
  window.noireInitTimeInputs?.($('#scheduleForm'));
  ...
  $('#scheduleForm').onsubmit=async ev=>{
    ...
    try{
      ev.target
        ...
        .forEach(x=>{
          if(window.noireNormalizeTime)
            x.value=noireNormalizeTime(x.value)||x.value
        });
      ...
      state.shifts=sh.shifts||[];
      state.scheduleEmployees=
        sh.employees||state.scheduleEmployees;
...
function applyRole(){
  ...
  if($('#addEmployeeBtn'))
    $('#addEmployeeBtn').hidden=!canManage;
  ...
  document.querySelectorAll('#adminNav button').forEach(b=>
    b.onclick=()=>{
      ...
      $('#viewSubtitle').textContent={
        ...
        ai:'Живой помощник по данным и задачам.'
      }[v]||''
    }
  )
}
...
async function askAI(q){
  ...
  try{
    ...
    box.innerHTML+=
      `<div class="ai-msg">
        Не удалось выполнить запрос: ${esc(e.message)}
      </div>`
  }
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('#addWaiterOrderBtn')?.addEventListener(
    'click',
    openWaiterOrder
  );

  $('#addDeliveryOrderBtn')?.addEventListener(
    'click',
    openDeliveryOrder
  );
  ...
  document.querySelectorAll('[data-go]').forEach(b=>
    b.onclick=()=>
      document
        .querySelector(
          '#adminNav [data-view="'+b.dataset.go+'"]'
        )
        ?.click()
  );

  $('#tableDate')?.addEventListener(
    'change',
    loadTables
  );

  $('#tableTime')?.addEventListener(
    'change',
    loadTables
  );

  $('#archivePrevMonthBtn')?.addEventListener(
    'click',
  ...
  $('#scheduleTodayMonth')?.addEventListener(
    'click',
    shiftToday
  );
  ...
  $('#scheduleEmployee')?.addEventListener(
    'change',
    renderSchedule
  );

  $('#scheduleRole')?.addEventListener(
    'change',
    renderSchedule
  );
